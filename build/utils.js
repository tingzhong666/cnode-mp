...
exports.cssLoaders = function (options) {
  ...
  // 递归遍历目录所有文件
  function stylusImport (dir) {
    let dirArr = []
    function ergodic (dir) {
      fs.readdirSync(dir, { withFileTypes: true }).forEach(item => {
        if (item.isFile()) dirArr.push(path.join(dir, item.name))
        else ergodic(path.join(dir, item.name))
      })
    }
    ergodic(dir)
    return dirArr
  ...
  return {
    ...
    stylus: generateLoaders('stylus', {
      import: [
        ...stylusImport(path.join(__dirname, '../src/stylus'))
      ]
    }),
